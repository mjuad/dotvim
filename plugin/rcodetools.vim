...
function! <SID>RCT_completion(findstart, base)
	...
	let command = "echo \"`date`\" >> /var/log/rbcomplete.log;rct-complete --completion-class-info --dev --fork --line=" .
		    \ line . " --column=" . column . " "
	let command = <SID>RCT_command_with_test_options(command) . s:rct_tmpfile . " 2>>/var/log/rbcomplete.log"
