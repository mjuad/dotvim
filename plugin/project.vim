...
function! s:Project(filename) " <<<
    ...
    if !exists("g:proj_running")
        ...
        function! ProjectNewFile(editcmd)
            let newfiledir =  s:GetHome(s:RecursivelyConstructDirectives(line('.')),'')
            let newfilename = input("Name for new file: ")
            let newfilepath = newfiledir . '/' . newfilename
            exec "wincmd l"
            exec a:editcmd . ' ' . newfilepath
        endfunction
        " Mappings <<<
        ...
        nnoremap <buffer> <silent> <LocalLeader>i :call ProjectNewFile('e')<CR>
        nnoremap <buffer> <silent> <LocalLeader>I :call ProjectNewFile('sp')<CR>
